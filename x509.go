...
type Certificate struct {
	...
	// of which are limited to int. If a certificate contains a policy which
...
func marshalSANs(dnsNames, emailAddresses []string, ipAddresses []net.IP, uris []*url.URL, otherNames []pkix.OtherName,
	registredID []pkix.RegisterID, directoryName []pkix.Name) (derBytes []byte, err error) {
	...
	for _, dirName := range directoryName {
		var val []byte
		val, err = asn1.Marshal(dirName.ToRDNSequence())
		if err != nil {
			return nil, err
		}
...
func buildCertExtensions(template *Certificate, subjectIsEmpty bool, authorityKeyId []byte, subjectKeyId []byte) (ret []pkix.Extension, err error) {
	...
	if (len(template.DNSNames) > 0 || len(template.EmailAddresses) > 0 || len(template.IPAddresses) > 0 || len(template.URIs) > 0 ||
		len(template.OtherNames) > 0 || len(template.RegisterIDs) > 0 || len(template.DirectoryNames) > 0) &&
		...
		ret[n].Value, err = marshalSANs(template.DNSNames, template.EmailAddresses, template.IPAddresses, template.URIs,
			template.OtherNames, template.RegisterIDs, template.DirectoryNames)
		if err != nil {
			return
		}
		n++
	...
	if (len(template.PermittedDNSDomains) > 0 || len(template.ExcludedDNSDomains) > 0 ||
		...
		serialiseConstraints := func(dns []string, ips []*net.IPNet, emails []string, uriDomains []string) (der []byte, err error) {
			...
			for _, name := range dns {
				...
				b.AddASN1(pkix.SEQUENCE, func(b *pkix.Builder) {
					b.AddASN1(pkix.Tag(2).ContextSpecific(), func(b *pkix.Builder) {
			...
			for _, ipNet := range ips {
				b.AddASN1(pkix.SEQUENCE, func(b *pkix.Builder) {
					b.AddASN1(pkix.Tag(7).ContextSpecific(), func(b *pkix.Builder) {
			...
			for _, email := range emails {
				...
				b.AddASN1(pkix.SEQUENCE, func(b *pkix.Builder) {
					b.AddASN1(pkix.Tag(1).ContextSpecific(), func(b *pkix.Builder) {
			...
			for _, uriDomain := range uriDomains {
				...
				b.AddASN1(pkix.SEQUENCE, func(b *pkix.Builder) {
					b.AddASN1(pkix.Tag(6).ContextSpecific(), func(b *pkix.Builder) {
		...
		b.AddASN1(pkix.SEQUENCE, func(b *pkix.Builder) {
			if len(permitted) > 0 {
				b.AddASN1(pkix.Tag(0).ContextSpecific().Constructed(), func(b *pkix.Builder) {
					b.AddBytes(permitted)
				})
			}

			if len(excluded) > 0 {
				b.AddASN1(pkix.Tag(1).ContextSpecific().Constructed(), func(b *pkix.Builder) {
					b.AddBytes(excluded)
				})
			}
		})
...
func marshalCertificatePolicies(policies []OID, policyIdentifiers []asn1.ObjectIdentifier) (pkix.Extension, error) {
	...
	b := pkix.NewBuilder(make([]byte, 0, 128))
	b.AddASN1(pkix.SEQUENCE, func(child *pkix.Builder) {
		for _, v := range policies {
			child.AddASN1(pkix.SEQUENCE, func(child *pkix.Builder) {
				child.AddASN1(pkix.OBJECT_IDENTIFIER, func(child *pkix.Builder) {
					if len(v.der) == 0 {
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	...
	for _, extension := range out.Extensions {
		...
		case extension.Id.Equal(oidExtensionSubjectAltName):
			out.DNSNames, out.EmailAddresses, out.IPAddresses, out.URIs, out.OtherNames,
				out.RegisterIDs, out.DirectoryNames, err = parseSANExtension(extension.Value)
			if err != nil {
				return nil, err
			}
		}
