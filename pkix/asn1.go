...
import (
	...
	goasn1 "github.com/pduveau/go-asn1"
)
...
func isValidOID(oid goasn1.ObjectIdentifier) bool {
	if len(oid) < 2 {
		return false
	}
...
func (b *Builder) AddASN1ObjectIdentifier(oid goasn1.ObjectIdentifier) {
...
// MarshalASN1 calls goasn1.Marshal on its input and appends the result if
// successful or records an error if one occurred.
func (b *Builder) MarshalASN1(v interface{}) {
	// NOTE(martinkr): This is somewhat of a hack to allow propagation of
	// goasn1.Marshal errors into Builder.err. N.B. if you call MarshalASN1 with a
	// value embedded into a struct, its tag information is lost.
	if b.err != nil {
		return
	}
	bytes, err := goasn1.Marshal(v)
	if err != nil {
		b.err = err
		return
	}
...
func (s *String) ReadASN1ObjectIdentifier(out *goasn1.ObjectIdentifier) bool {
	var bytes String
	...
	if v < 80 {
		components[0] = int(v / 40)
		components[1] = int(v % 40)
	} else {
		components[0] = 2
		components[1] = int(v - 80)
	}
...
// It reports whether the read was successful.
func (s *String) ReadASN1BitString(out *goasn1.BitString) bool {
	var bytes String
