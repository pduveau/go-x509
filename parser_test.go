...
func TestParseASN1String(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: asn1.T61String("¿a?"),
		},
		{
			name:     "PrintableString",
			tag:      pkix.PrintableString,
			value:    []byte{80, 81, 82},
			expected: string("PQR"),
		},
		{
		...
		{
			name:     "BMPString",
			tag:      pkix.Tag(asn1.TagBMPString),
			value:    []byte{80, 81},
			expected: "偑",
		},
		{
		...
		{
			name:     "NumericString",
			tag:      pkix.Tag(asn1.TagNumericString),
			value:    []byte{49, 50},
			expected: asn1.NUMERICString("12"),
		},
		{
